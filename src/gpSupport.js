...
// 创建文件选择器 UI
function createFilePickerUI(files, onSelect, onCancel) {
	...
	title.textContent = isChinese ? '选择文件' : 'Select File';
	title.style.cssText = `
        margin: 0 0 15px 0;
        font-size: 18px;
        color: #333;
	...
	if (files.length === 0) {
		...
		emptyMsg.textContent = isChinese ? '没有找到文件' : 'No files found';
...
async function GP_ReadFile(ext) {
	...
	if (typeof window.Capacitor !== 'undefined' && window.Capacitor.isNativePlatform()) {
		try {
			...
			const isChinese = navigator.language.toLowerCase().startsWith('zh');
			const messages = {
				readSuccess: isChinese ? '文件读取成功' : 'File read successfully',
				readError: isChinese ? '读取文件失败: ' : 'Failed to read file: ',
				cancelOpen: isChinese ? '用户取消打开' : 'User cancelled open',
				noFiles: isChinese ? '目录下没有文件' : 'No files in directory'
			};

			// 1. 直接读取 Documents 目录下的所有文件
			let fileList = [];
			try {
				...
				// 过滤指定扩展名的文件
				fileList = result.files.filter(file => {
					// 过滤掉目录
				...
				async (selectedFile) => {
					try {
						...
						if (result.data.startsWith('data:')) {
							...
							contents = encoder.encode(result.data).buffer;
						}

						// 添加到 droppedFiles
						GP.droppedFiles.push({
							name: toUTF8Array(selectedFile.name),
							contents: contents
						});

						console.log(messages.readSuccess + ': ' + selectedFile.name);
					} catch (e) {
						console.error('Error reading file:', e);
						alert(messages.readError + e.message);
					}
				},
				// 取消的回调
				() => {
					console.log(messages.cancelOpen);
				}
			);
			...
			const isChinese = navigator.language.toLowerCase().startsWith('zh');
			const errorMsg = isChinese ? '打开文件选择器失败: ' : 'Failed to open file picker: ';
	...
	if (typeof window.Capacitor !== 'undefined' && window.Capacitor.isNativePlatform()) {
		try {
			...
			const messages = {
				...
				saveSuccess: isChinese ? '文件已保存到 CoCube/' : 'File saved to CoCube/',
